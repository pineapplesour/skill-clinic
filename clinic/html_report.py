...
def _header(report: dict) -> str:
    ...
    return (
        ...
        + _security_pill(report)
        + f'<ul class="counts">{"".join(tiles)}</ul>'
        + "</header>"
    )


def _security_pill(report: dict) -> str:
    """`SECURITY: SUSPICIOUS (2 high)` beside the verdict pill."""
    sec = report.get("security") or {}
    high, medium, low = sec.get("high", 0), sec.get("medium", 0), sec.get("low", 0)
    if high:
        label, key = f"SECURITY: SUSPICIOUS ({high} high)", "high"
    elif medium:
        label, key = f"SECURITY: SUSPICIOUS ({medium} medium)", "medium"
    elif low:
        label, key = f"SECURITY: {low} low-severity note(s)", "low"
    else:
        label, key = "SECURITY: clean", "info"
    fg, bg, border = _SEV[key]
    return (f'<span class="verdict" style="margin-left:.5rem;color:{fg};background:{bg};'
            f'border-color:{border}">{_esc(label)}</span>')
...
_SEV = {
    "high": ("#8a1010", "#ffe3e3", "#f3b4b4"),
    "medium": ("#8a5a00", "#fff3d6", "#f0d69a"),
    "low": ("#3a4a6a", "#e8eeff", "#c4d2f0"),
    "info": ("#4a4a55", "#eeeef2", "#d8d8e0"),
}


def _security(report: dict) -> str:
    """'Security observations' card: what flagged steps actually tried to do."""
    steps = report.get("steps") or []
    flagged = [s for s in steps if s.get("security")]
    intro = ("Every step below was <strong>executed</strong> in a throwaway Daytona sandbox "
             "with arbitrary egress blocked, so this is what the skill <em>tried</em> to do "
             "&mdash; not a static guess. Observations only: the verdict is unchanged.")
    if not flagged:
        return ('<section class="card"><h2>Security observations</h2>'
                '<p class="muted">No injected or suspicious behaviour was observed '
                "in any step.</p></section>")
    rows = []
    for step in flagged:
        for finding in step["security"]:
            fg, bg, border = _SEV.get(finding.get("severity", ""), _SEV["info"])
            rows.append(
                "<tr>"
                f'<td class="num">{_esc(step.get("index"))}</td>'
                f'<td>{_esc(step.get("title"))}</td>'
                f'<td><span class="pill" style="color:{fg};background:{bg};'
                f'border-color:{border}">{_esc(finding.get("severity"))}</span></td>'
                f'<td class="cat">{_esc(finding.get("rule"))}</td>'
                f'<td><code>{_esc(finding.get("evidence"))}</code></td>'
                f'<td class="muted">{_esc(finding.get("note"))}</td>'
                "</tr>")
    return (
        '<section class="card"><h2>Security observations</h2>'
        f'<p class="muted">{intro}</p><table>'
        '<thead><tr><th class="num">#</th><th>Step</th><th>Severity</th><th>Rule</th>'
        '<th>Evidence</th><th>What it means</th></tr></thead>'
        f'<tbody>{"".join(rows)}</tbody></table></section>'
    )
